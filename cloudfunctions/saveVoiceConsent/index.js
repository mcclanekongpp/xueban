...
function createConsentId(userId, subjectId) {
  const digest = crypto
    .createHash('sha256')
    .update(`${userId}|${subjectId}|${CONSENT_VERSION}`)
    .digest('hex')
    .slice(0, 32)
    .toUpperCase()
  return `VC_${digest}`
...
exports.main = async (event = {}) => {
  ...
  try {
    ...
    const activeResult = await db.collection('voice_consents').where({
    ...
    if (activeResult.data.length === 1) {
      return {
        ...
        has_consent: true,
        reused: true,
        consent_id: activeResult.data[0].consent_id || activeResult.data[0]._id,
        subject_id: subjectId,
    ...
    const consentId = createConsentId(authorization.user.user_id, subjectId)
    const now = db.serverDate()
    await db.collection('voice_consents').doc(consentId).set({
      data: {
    ...
    console.error('saveVoiceConsent error:', error)
    return {
      success: false,
      code: 'SAVE_VOICE_CONSENT_ERROR',
      message: error.message || '录音授权保存失败'
    }
  }
}
